/*
  ...
  Portions Copyright 2009 Novell Inc. All rights reserved.
    ...
    */
    if(machine == EM_MIPS) {
        /* This is really wierd. Treat this very specially. 
           ...
           No Elf64 BE MIPS is at hand as this is written.
         */
    ...
    struct Dwarf_Elf_Rela *relap)
{
    ...
    for (i=0; i < num_relocations; i++) {
       get_relocations(data, i,machine, &(relap[i]));
    }
    ...
    int *error)
{
    ...
    if ((relocation_section_size != 0)) {
        ...
        *relas = malloc(bytescount);
        ...
        get_relocations_array(is_64bit,machine, relocation_section, 
            *nrelas, *relas);
    }
    return(DW_DLV_OK);
}
