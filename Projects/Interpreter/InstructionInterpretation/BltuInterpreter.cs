...
{
    class BltuInterpreter : IInstructionInterpreter
    {
        public bool InterpretInstruction(RuntimeContext ctx, int[] argList)
        {
            if (argList.Length != 3)
            {
                throw new InvalidOperationException("Malformed BLTU instruction - expected three parameters; received " + argList.Length);
            }
