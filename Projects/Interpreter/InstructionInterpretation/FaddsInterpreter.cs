...
{
   internal class FaddsInterpreter : IInstructionInterpreter
   {
      public bool InterpretInstruction(RuntimeContext ctx, int[] argList)
      {
         if (argList.Length != 3)
         {
            throw new InvalidOperationException("Malformed FADD.S instruction - expected three parameters; received " + argList.Length);
         }
