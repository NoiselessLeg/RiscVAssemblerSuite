...
{
   class FltPtCompareInstructionStringifier : IParameterStringifier
   {
      public FltPtCompareInstructionStringifier(string instructionName)
      {
      ...
      public string GetFormattedInstruction(int currPgrmCtr, DisassembledInstruction inst, ReverseSymbolTable symTable)
      {
         ...
         {
            throw new ArgumentException("Floating point R-type instruction expected 3 arguments, received " + inst.Parameters.Count());
         }

         string rd = ReverseRegisterMap.GetStringifiedRegisterValue(inst.Parameters.ElementAt(0));
         string rs1 = ReverseRegisterMap.GetStringifiedFloatingPtRegisterValue(inst.Parameters.ElementAt(1));
         string rs2 = ReverseRegisterMap.GetStringifiedFloatingPtRegisterValue(inst.Parameters.ElementAt(2));
